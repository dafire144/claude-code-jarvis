...
- `hud-native/` — telinha nativa do Windows (WinForms). Recompilar: `csc -nologo -target:winexe -out:jarvis-hud-wf.exe -r:System.Windows.Forms.dll -r:System.Drawing.dll JarvisHudWF.cs HudLayout.cs FanoutHud.cs` (csc do .NET Framework, C# 5 — sem interpolação `$""` nem `?.`). QA sem abrir janela: `jarvis-hud-wf.exe --shot saida.png` (e `--shot saida.png fable`); cinemáticas frame a frame: `--shot-boot saida.png 0.7 [fable]` (ignição) e `--shot-shut saida.png 0.6` (desligamento). O HUD Electron tem os mesmos modos (`electron . --shot ...`).
