...
- **Minimize to a mini-reactor** — a **–** button collapses the whole panel into a tiny always-on-top **reactor capsule** (live core + session name + status) that tucks into the corner; click it and it **expands back** with a cinematic implosion/bloom morph. And a tiny **floating toggle button** lives beside the first capsule of the dock — one click **hides every HUD window** ("show desktop" style, the button becomes a pill with a live session count), one more click brings them all back exactly where they were.
